...
Any vector space equipped with an inner product is a spacetime. Given a reference vector $r$ (which defines an "arrow of time") the inner product defines a partial ordering on points in the vector space so that $y > x$ iff $(y-x).(y-x) > 0$ and $r.(y-x) > 0$. Given a point $x$, we call the set of points $\lbrace y : y > x\rbrace$ the *future light cone* of $x$.
